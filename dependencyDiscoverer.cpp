...
struct hashmap {
  private:
    std::unordered_map<std::string, std::list<std::string>> hashmap;
    std:: mutex mutex;
  public:
    std::list<std::string>* get(std::string f){
      std::unique_lock<std::mutex> lock(mutex);
      return &hashmap[f];
    }
    std::unordered_map<std::string, std::list<std::string>>::iterator find(std::string name){
      std::unique_lock<std::mutex> lock(mutex);
      return hashmap.find(name);
    }
...
int workQIterator(){
  ...
  while ( true ) {
    std::string filename = workQ.frontpopfront();
    if (filename == "THREAD_RETURN"){
      return 0;
    }
...
int main(int argc, char *argv[]) {
  ...
  for (i=0; i<threads; i++){
    threadVect.emplace_back(workQIterator);
  }
